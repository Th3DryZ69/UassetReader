...
public class FKismetArchive : FArchive
{
	...
	public FKismetArchive(string name, byte[] data, IPackage owner, VersionContainer? versions = null)
		: base(versions)
	{
		_data = data;
		Name = name;
	...
	{
		int index = Index;
		object obj = (EExprToken)Read<byte>() switch
		{
		...
		((KismetExpression)obj).StatementIndex = index;
		return (KismetExpression)obj;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public string XFERSTRING()
	{
		int num = Array.IndexOf(_data, (byte)0, (int)Position);
		if (num == -1)
		{
			throw new ParserException("Couldn't find end of the string");
		}
		return Encoding.ASCII.GetString(ReadBytes(num - (int)Position));
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public string XFERUNICODESTRING()
	{
		Span<byte> span = _data.AsSpan((int)Position);
		Span<byte> span2 = stackalloc byte[2];
		int num = span.IndexOf(span2);
		if (num == -1)
		{
			throw new ParserException("Couldn't find end of the string");
		}
	...
	public KismetExpression[] ReadExpressionArray(EExprToken endToken)
	{
		...
		while (kismetExpression == null || kismetExpression.Token != endToken)
		{
			if (kismetExpression != null)
			{
				list.Add(kismetExpression);
			}
			kismetExpression = ReadExpression();
		}
		return list.ToArray();
	}
	...
	{
		return new FKismetArchive(Name, _data, Owner, Versions)
		{
			Position = Position
		};
	}
